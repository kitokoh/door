<?php
if (doors_get_option('doors_search_icon','off') != "off") { ?>
		...
		<div class="overlay-search hide">
			<form method="get" class="search-form" action="<?php echo esc_url(home_url( '/' )); ?>">
				<label>
					...
					<input type="search" class="search-field" placeholder="<?php echo esc_attr__( 'Search','doors' ) ?>" value="<?php echo get_search_query() ?>" name="s" title="<?php echo esc_attr__( 'Search for:','doors' ) ?>" />
				</label>
				<input type="submit" class="search-submit" value="<?php echo esc_attr__( 'Search','doors' ) ?>" />
			</form>
		</div>
	</div>
<?php }