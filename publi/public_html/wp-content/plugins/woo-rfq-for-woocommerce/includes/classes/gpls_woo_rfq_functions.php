...
.site-header-cart *{
    ...
    visibility:visible !important;
                ...
                .woocommerce-Price-amount,.from, .price, .product-selector__price,
                  .total, .bundle_price,.wc-pao-col2,.single-product div.product form.cart .single_add_to_cart_button {visibility: collapse}
...
";
                ...
                echo "<div class='gpls_script' style='display: none'><style>.single_add_to_cart_button { visibility: collapse} #woo_pp_ec_button_product {display:none !important} .gpls_rfq_set{visibility: visible !important;}</style></div>";
...
";
    ...
    function gpls_woo_rfq_woocommerce_add_to_cart($cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data)
    {
        ...
        if ($checkout_option == "rfq") {
            ...
            //$rfq_enable_rfq_checkout = get_post_meta($product_id, '_gpls_woo_rfq_rfq_enable', true);
            //$rfq_enable = get_post_meta($product->get_id(), '_gpls_woo_rfq_rfq_enable', true);
            // $rfq_enable = apply_filters('gpls_rfq_enable', $rfq_enable, $product->get_id());
      ...
      if(($rfq_enable == 'yes' && isset($_REQUEST["rfq_product_id"]) && $_REQUEST["rfq_product_id"] != "-1")||($rfq_enable == 'yes'&& isset($_GET["add-to-cart"])))
        {
          //  $is_an_rfq = true;
        }


     if (($rfq_enable == 'yes' && !empty($_REQUEST["rfq_product_id"]) && $_REQUEST["rfq_product_id"] != "-1")||($rfq_enable == 'yes'&& isset($_GET["add-to-cart"]))) {
            $is_an_rfq = true;
        }
