...
class FetchProducts {
	...
	public function fetch_products() {
		if ( ! wp_verify_nonce( $_POST['nonce'], 'swptls_fetch_products_nonce_action' ) ) {
			wp_send_json_error([
	...
	public static function get_other_products() {
		...
		if ( $data && ! is_wp_error( $data ) ) {
			...
			$recommended_plugins['webinar-and-video-conference-with-jitsi-meet']->short_description = esc_html__( 'Webinar and Video Conference with Jitsi Meet.', 'sheetstowptable' );
		}
		...
		if ( $data && ! is_wp_error( $data ) ) {
			...
			$recommended_plugins['chat-widgets-for-multivendor-marketplaces']->name              = __( 'Chat Widgets for Multivendor Marketplaces', 'sheetstowptable' );
			$recommended_plugins['chat-widgets-for-multivendor-marketplaces']->short_description = esc_html__( 'Chat with your customers via their favorite channels. Show a chat button on the bottom…', 'sheetstowptable' );
		}
		...
		if ( $data && ! is_wp_error( $data ) ) {
			...
			$recommended_plugins['flexiaddons']->name              = __( 'Flexi Addons for Elementor', 'sheetstowptable' );
			$recommended_plugins['flexiaddons']->short_description = esc_html__( 'A collection of premium quality & highly customizable addons or modules for use in Elementor page builder.', 'sheetstowptable' );
		}
